...
// ══════════════════════════════════════════
// CHARGEMENT DONNÉES - VERSION CORRIGÉE
// ══════════════════════════════════════════
async function loadAll() {
  ...
  const fetchSafe = async (url) => {
    try {
      ...
      console.warn('Erreur fetch:', url, e);
      return [];
    }
  };

  try {
    D.projects  = await fetchSafe(`${API}/projects/`);
    D.members   = await fetchSafe(`${API}/members/`);
    D.workItems = await fetchSafe(`${API}/workitems/`);
    D.tenders   = await fetchSafe(`${API}/tenders/`);
    D.sprints   = await fetchSafe(`${API}/sprints/`);

    // Dashboard stats séparé
    try {
      const dsRes = await apiFetch(`${API}/dashboard/`);
      dashStats = (dsRes && dsRes.ok) ? await dsRes.json() : buildLocalStats();
    } catch(e) {
      dashStats = buildLocalStats();
    }

    console.log('✅ Données:', D.projects.length, 'projets,', D.members.length, 'membres');
    renderSidebar();
    ...
    renderSidebar();
    renderPage(currentPage);
    toast('Données partiellement chargées', 'info', 'ℹ');
  } finally {
    if (loading) loading.classList.remove('active');
  }
}

// Stats calculées localement si /api/dashboard/ plante
function buildLocalStats() {
  return {
    active_items:    D.workItems.filter(w => w.status === 'En cours').length,
    active_projects: D.projects.filter(p => p.status === 'En cours').length,
    done_items:      D.workItems.filter(w => w.status === 'Terminé').length,
    total_tenders:   D.tenders.length,
    backlog_items:   D.workItems.filter(w => w.status === 'Backlog').length,
    total_members:   D.members.length,
    items_by_status: {
      'Backlog':  D.workItems.filter(w => w.status === 'Backlog').length,
      'A faire':  D.workItems.filter(w => w.status === 'A faire').length,
      'En cours': D.workItems.filter(w => w.status === 'En cours').length,
      'Review':   D.workItems.filter(w => w.status === 'Review').length,
      'Terminé':  D.workItems.filter(w => w.status === 'Terminé').length,
    },
    ao_by_status: {
      'Détection':    D.tenders.filter(t => t.status === 'Détection').length,
      'Qualification':D.tenders.filter(t => t.status === 'Qualification').length,
      'Préparation':  D.tenders.filter(t => t.status === 'Préparation').length,
      'Soumis':       D.tenders.filter(t => t.status === 'Soumis').length,
      'Gagné':        D.tenders.filter(t => t.status === 'Gagné').length,
      'Perdu':        D.tenders.filter(t => t.status === 'Perdu').length,
    },
    velocity_current: 0,
    active_sprint: D.sprints.find(s => s.status === 'En cours') || null,
    recent_projects: D.projects.slice(0, 5),
  };
}
